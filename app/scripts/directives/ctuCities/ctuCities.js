(function () {
    cityTripApp.directive('ctuCities',['databaseService', 'Page', function (databaseService, Page)  {
        return {
            restrict: 'E',
            replace: true,
            scope: {},
            templateUrl: 'app/scripts/directives/ctuCities/ctuCities.html',
            css: 'app/scripts/directives/ctuCities/ctuCities.css',
            link: function () {
                
            },
            controller: function ($scope,$rootScope, $location, $state, $window) {
                Page.setTitle("Cities");
                var index = [ ];
                var self = { };
                ...
                self.init = function () {
                    ...
                    databaseService.getCities()
                        .then(function success(data) {
                                $scope.cityInfo = data;
                                randomBackground($scope.cityInfo);
                                setNavigationData($scope.cityInfo);
                            },
                            function error() {
                ...
                function randomBackground(citydata) {
                    var backgrounds = [];
                    for (var i = 0; i < citydata.length; i++) {
                        backgrounds.push({
                            name: citydata[i].name,
                            nickname: citydata[i].nickname,
                            image: citydata[i].image,
                            color: randomColor(),
                            colspan: randomSpan(),
                            rowspan: randomSpan()
                        });
                    }
                    $scope.tileBackgrounds = backgrounds;
                }

                $scope.navigateTo = function ($index, background) {
                    var id = { },
                        city;
                    for(var i = 0; i < index.length; i++ ){
                        if(index[i].name === background.name) {
                            city = index[i].name;
                            id = index[i]._id;
                            $scope.$emit('ctuCities.cityName', city);
                            $state.go('city', {obj: id});
                        }
                    }
                };

                function setNavigationData(citydata) {
                    for(var i = 0; i < citydata.length; i++) {
                        if(citydata[i].photoset_id !== undefined) {
                            index.push(citydata[i]);
                        }
                    }
                }
